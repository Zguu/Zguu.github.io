...
수식 (7.5)에서 보여지는 이 근사 값은 $N$이 커지면 커질수록 더 정확해 질 것이다. 수식 (7.5)는 일반적 원칙의 특별한 케이스일 뿐이다. 모든 함수 $f(\theta)$에 대하여, 확률 분포 $$p(\theta)$$에 의해 weighted된 해당 함수의 적분은 샘플링된 포인트 들에서의 함숫값의 평균으로 근사될 수 있으며 수학적으로는 아래와 같이 쓴다.<br>
