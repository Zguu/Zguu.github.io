...
$${n}_j/N \approx \Del \theta p({\theta}_j)$$가 성립하게 된다. 그래서 다음과 같은 식이 성립하게 된다: $$\int d\theta f(\theta) p(\theta) \approx \sum_{j}[\Delta \theta p({\theta}_j)] f({\theta}_j) \approx \sum_{j} [n_j / N] f({\theta}_j) = 1/N \sum_j n_j f({\theta}_j)$$. 
