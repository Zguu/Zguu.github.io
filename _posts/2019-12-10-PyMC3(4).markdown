...
$\ $특히 비율이나 확률 $p$를 $$Beta(\alpha,\beta)$$로 설정하면 편할 수 있다. 이항분포에서 얻게되는 데이터를 우리가 관측하고 있다고 하자.$$X \thicksim Binomial(N,p)$$이고 우리가 $p$를 모르는 상태이다. 이 경우에 우리의 사후분포는 다시 베타분포를 따르게 된다.
