...
해당 공식에서 분모 부분이 $$B(z+a,N-z+b)$$로 치환되는 부분이 잘 이해가 안될 수 있는데, 해당 부분은 수학적 유도가 어려우니 궁금하면 따로 찾아보자. 수식 (5.7)이 알려주는 것은 아주 간단하다. 우리가 사전 분포 $$beta(\theta;a,b)$$를 아는 상태에서, 추가적인 시행 (N번의 동전을 던져 z번의 head를 얻는)을 통해 우리의 믿음을 업데이트 하면, 사후 분포는 $$beta(\theta;z+a,N-z+b)$$가 된다는 것이다. 이렇게 간단한 업데이트 룰이 베이즈 추론의 수학적 접근의 미학이라고 볼 수 있다. 뿐만 아니라, 사전 평균과 사후 평균의 관계에 대해서도 생각을 해보자.$\theta$의 사전 평균은 $$a/(a+b)$$이다. 사후 평균은 $$(z+a)/[(z+a)+(N-z+b)] = (z+a)/(N+a+b)$$이다.
>위의 사후 평균이 왜 위 같은 형태로 계산이 되는 것인지에 대한 이유를 잘 생각해보자. conjugate에서 힌트를 얻을 수 있다.

사후 평균은 수식적으로 사전 평균 $$a/(a+b)$$와 데이터가 보여주는 비율 $$z/N$$의 weighted average로 표현할 수 있다.<br>
!['Imgur2'](https://imgur.com/9KTvmCx.png)
수식 (5.8)에서 당연하게 알 수 있는 것은, 사후 평균은 항상 사전 평균과 데이터의 비율 사이에 존재한다는 것이다.

# Three inferential goals
## Estimating the binomial Proportion
!['Imgur2'](https://imgur.com/85EuTTn.png)
위의 Figure 5.2 에서 사후 분포의 HDI 범위에 대해 잘 생각해보자. 현재 왼쪽 패널에 있는 그래프들과 오른쪽 패널에 있는 그래프들은 같은 데이터를 사용했고, 그러므로 같은 likelihood 그래프를 보여준다. 왼쪽 패널의 사전분포는 uniform하므로, likelihood 함수를 통해 사후분포를 얻어내도 완전히 같은 형태의 모양을 보여준다. 반면에, 오른쪽 패널의 사전분포는 uniform하지 않고 어느 정도의 정보량을 포함하고 있기에, 사후분포의 형태가 달라진다. 또한 사후 분포의 95% HDI가 더욱 좁은 구간에 존재하게 되며 불확실성이 덜하다고 볼 수 있다.
## Predicting Data
datum 값 $y$의 예측된 확률은 모든 가능한 파라미터 값들에 대해 평균을 취함으로써 얻을 수 있다 :$$p(y)\ =\ \int d\theta p(y|\theta)p(\theta)$$. 파라미터 값의 믿음에 해당하는 $p(\theta)$는 현재의 사후 믿음이며 이것은 지금까지의 관측된 데이터를 포함하기에 다음과 같이 표현할 수 있다 $$p(\theta|z,N)$$.<br>
현재 모델의 적용에 있어서 heads 확률 예측은 특히나 간단하다.($$p(y=1|\theta)=\theta$$이기 때문) 따라서, 아래의 식을 얻는다.
!['Imgur2'](https://imgur.com/SoqfqgF.png)
해당 식은 지금까지 설명한 내용들의 연장선이며, 직관적 이해가 가능한 부분이기에 더 이상 설명하지 않도록 한다.
## Model comparison
