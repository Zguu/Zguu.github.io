...
$\ $<center>$$\hat{\theta_{MAP}} = argmax\left\{Posteriori\right\} \ \= argmax {P(\theta|D)\ \= argmax {P(D|\theta)P(\theta)/P(D)} \ \= argmax {P(D|\theta)P(\theta)} = argmax\left\{log P(D|\theta) + log P(\theta)\right\}
$$</center><br>
